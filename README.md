<div align="center">*Simplifying Complexity, Building Solutions*</div>

### Currently
- 🎨 Obsessed with creating delightful developer experiences
- 🏛️ Establishing a foundation that ties all projects into one platform
  - 🧱 Building a flexible and accessible UI component library
  - ⚡ Planning a new JavaScript framework
  - 🖼️ Crafting a sleek and expressive icon library
  - 📊 Designing a refined and uplifting charting system

### Skills and Experience
* 🧩 Angular, React, Svelte, Nextjs, Vuejs
* ⚙️ C#, C++, Java, Rust, .NET Core, COBOL, MSSQL
* 💻 HTML, SCSS, TypeScript, Bootstrap, TailwindCSS
* 🌐 WordPress, Webflow, Canva, Figma
* ⚒️ Visual Studio, Visual Studio Code, GitHub, GitLab, Postman, LocalWP

### GitHub Stats
![GitHub Daily Streak](https://streak-stats.demolab.com?user=andrejkoller&theme=dark&hide_border=true&background=000000&exclude_days=Sun%2CSat")
